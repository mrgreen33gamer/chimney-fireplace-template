  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        if (p.flake) {
          ctx.strokeStyle = '#9a3412'; ctx.lineWidth = 0.6;
          ctx.translate(p.x, p.y);
          for (let i = 0; i < 6; i++) { ctx.rotate(Math.PI / 3); ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, p.r * 3.2); ctx.stroke(); }
        } else { ctx.fillStyle = '#9a3412'; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill(); }
        ctx.restore();
        p.x += p.vx; p.y += p.vy;
  ...
  return (
    <div className={styles.thermo} aria-hidden="true">
      ...
      <div className={styles.thermoLabels}>
        <span className={styles.thermoTop}>Safe</span>
...
const CHIPS = ['Same-Day Service', 'No Contracts', 'CSIA-Certified', '20+ Yrs Local', 'Clean Sweep Guarantee'];
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.content}>
          <motion.div className={styles.badge}
            ...
            Waco&apos;s Most Trusted Chimney Pros — Since 2006
          </motion.div>

          <motion.h1 className={styles.headline}
            initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.1 }}>
            Chimneys Swept.<br />Fires Made Safe.<br />
            <span className={styles.accentLine}>EmberStack Chimney & Fireplace.</span>
          </motion.h1>

          <motion.p className={styles.sub}
            initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, delay: 0.22 }}>
            Chimney Sweep · Repair · Fireplace Install. Flat-rate pricing. Same-day service.
            Clean Sweep Guarantee · Fully Insured. Serving Waco and Central Texas with CSIA-certified sweeps.
          </motion.p>

          <motion.div className={styles.ctaRow}
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.34 }}>
            <a href="tel:+12548404040" className={styles.ctaPrimary}>
